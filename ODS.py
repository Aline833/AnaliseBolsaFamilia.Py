...
caminho_arquivo = r"C:\Users\aline\OneDrive\Área de Trabalho\Faculdade -Ciencias de Dados\Extensionista\2\POP2024_20241230.xls"
...
df_bolsa["proporcao_familias"] = (
    df_bolsa["qtd_familias_beneficiarias_bolsa_familia_s"] / df_bolsa["Populacao"]
)
...
with col1: 
    ...
    grafico = sns.barplot(data=top_valor, x="valor_repassado_bolsa_familia_s", y="Municipio", palette="viridis", ax=ax1)
...
def formatar_valor(valor):
        if valor >= 1_000_000:
            ...
            return f"{valor:.0f}"
with col2:
    st.subheader("🔹 Top 10 municípios com maior número de famílias beneficiadas")
    top_familias = df_bolsa.nlargest(10, "qtd_familias_beneficiarias_bolsa_familia_s")
    fig2, ax2 = plt.subplots(figsize=(10, 5))
    grafico2=sns.barplot(data=top_familias, x="qtd_familias_beneficiarias_bolsa_familia_s", y="Municipio", palette="magma", ax=ax2)
...
with col3:
    ...
    top_prop = df_bolsa.dropna(subset=["proporcao_familias"]).nlargest(10, "proporcao_familias")
    fig3, ax3 = plt.subplots(figsize=(10, 5))
    grafico3=sns.barplot(data=top_prop, x="proporcao_familias", y="Municipio", palette="coolwarm", ax=ax3)
...
with col4:  
    ...
    municipio_mean = df_bolsa.groupby("Municipio")["pbf_vlr_medio_benef_f"].mean().sort_values(ascending =False)
    #Selecionar os 10 municípios com maior valor médio
    top10_municipios = municipio_mean.head(10)
    # Criar o gráfico
    fig4, ax4 = plt.subplots(figsize=(10, 5))
    grafico4 = sns.barplot(x=top10_municipios.values,y=top10_municipios.index, palette ="cubehelix",ax = ax4)
